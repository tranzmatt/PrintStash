...
def update_policy(
    ...
    *,
    ...
    policy.next_due_at = next_slot(policy, now) if policy.enabled else None
...
def storage_generation(session: Session) -> str:
    ...
    markers = session.exec(select(RestoreMarker).order_by(RestoreMarker.id)).all()
    payload = [
...
def admit_run(
    ...
    *,
    ...
    now = ensure_utc(now or utcnow())
    active = session.exec(
        select(VaultAuditRun).where(VaultAuditRun.state.in_(ACTIVE_STATES))
    ).first()
...
def claim_due(
    ...
    policies = session.exec(
        ...
        .where(VaultAuditPolicy.enabled.is_(True), VaultAuditPolicy.paused.is_(False))
        .order_by(VaultAuditPolicy.next_due_at)
    ).all()  # noqa: E712
