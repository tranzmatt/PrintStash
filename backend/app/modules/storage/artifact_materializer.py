...
class ArtifactMaterializer:
    def __init__(
        ...
        if not marker.exists():
            ...
            marker.write_text("printstash-artifact-cache-v1\n")
        if (
            marker.is_symlink()
    ...
    def _metric(self, db: sqlite3.Connection, name: str) -> None:
        db.execute(
            "INSERT INTO metrics VALUES (?,1) ON CONFLICT(name) DO UPDATE SET value=MIN(value+1,9223372036854775806)",
            (name,),
        )
    ...
    def reconcile(self) -> None:
        with self._transaction() as db:
            ...
            known.update(
                f"{str(r[0]).removeprefix('revoked:')}.tmp"
    ...
    def acquire(self, representation: Representation) -> CacheLease | None:
        ...
        with self._transaction() as db:
            ...
            db.execute(
                "UPDATE entries SET used=?,hits=hits+1 WHERE key=?",
    ...
    def status(self) -> dict[str, int]:
        with self._transaction() as db:
            ...
            return {
                ...
                **{r[0]: r[1] for r in db.execute("SELECT * FROM metrics")},
            }
...
class CacheFill:
    ...
    def complete(self) -> CacheLease | None:
        ...
        with self.cache._transaction() as db:
            ...
            db.execute(
                "INSERT INTO entries(key,size,digest,inode,mtime,used) VALUES (?,?,?,?,?,?)",
                (
