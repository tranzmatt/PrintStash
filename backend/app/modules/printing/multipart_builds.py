...
def require(
    ...
    if build is None:
        raise OperationError("build_not_found", kind=ErrorKind.NOT_FOUND)
...
def _claim(session: Session, build: MultipartBuild, version: int) -> None:
    if build.archived_at is not None:
        raise OperationError("build_archived", kind=ErrorKind.CONFLICT)
    ...
    if changed.rowcount != 1:
        raise OperationError("build_version_conflict", kind=ErrorKind.CONFLICT)
...
def _part(session: Session, build: MultipartBuild, part_id: int) -> MultipartBuildPart:
    ...
    if part is None or part.build_id != build.id:
        raise OperationError("build_part_not_found", kind=ErrorKind.NOT_FOUND)
    return part
...
def create(session: Session, user: User, payload: BuildCreate) -> MultipartBuild:
    ...
    if not parts:
        raise OperationError("build_composition_empty", kind=ErrorKind.INVALID)
    ...
    if not build.name:
        raise OperationError("build_name_empty", kind=ErrorKind.INVALID)
    session.add(build)
    session.flush()
    for part in parts:
        choices = []
...
def select_revision(
    ...
    if selected is None or (
        ...
        raise OperationError("build_choice_invalid", kind=ErrorKind.INVALID)
    model_id = selected["model_id"]
    model = _model(session, user, model_id)
    if model is None:
        raise OperationError("build_model_unavailable", kind=ErrorKind.INVALID)
    if (
        payload.revision_id is not None
        and _revision(session, model, payload.revision_id) is None
    ):
        raise OperationError("build_revision_invalid", kind=ErrorKind.INVALID)
...
def enqueue(
    ...
    revision = _revision(session, model, part.revision_id)
    if revision is None:
        raise OperationError("build_revision_required", kind=ErrorKind.CONFLICT)
    ...
    if available == 0:
        raise OperationError("build_no_unreserved_units", kind=ErrorKind.CONFLICT)
    count = payload.job_count or math.ceil(available / payload.units_per_job)
    if count > settings.fleet_batch_max_quantity:
        raise OperationError("batch_quantity_exceeds_limit", kind=ErrorKind.INVALID)
    if count * payload.units_per_job > available and not payload.confirm_excess:
        raise OperationError(
            "build_excess_confirmation_required", kind=ErrorKind.CONFLICT
        )
...
def _receipt_matches(
    ...
    if (
        ...
        raise OperationError("build_idempotency_conflict", kind=ErrorKind.CONFLICT)
    return True
...
def confirm(
    ...
    if attempt is None:
        raise OperationError("build_attempt_not_found", kind=ErrorKind.NOT_FOUND)
    ...
    if payload.valid_units > attempt.planned_units:
        raise OperationError("build_valid_units_exceed_planned", kind=ErrorKind.INVALID)
    job = session.get(PrintJob, attempt.job_id) if attempt.job_id else None
    if job and job.state not in TERMINAL:
        raise OperationError("build_job_not_finished", kind=ErrorKind.CONFLICT)
    changed = session.execute(
    ...
    if changed.rowcount != 1:
        if _receipt_matches(session.exec(receipt_query).first(), payload):
            return build
        raise OperationError("build_result_version_conflict", kind=ErrorKind.CONFLICT)
    session.add(
        MultipartBuildConfirmation(
