...
def find_backup_witness() -> BackupWitness | None:
    ...
    now = utcnow()
    for meta in backup_catalogue.list_backup_sources():
        ...
        if evidence is None:
            continue
        verification = backup_verification.verify_backup(
...
def _source_identity_evidence(meta) -> tuple[dict, dict] | None:
    ...
    if meta.location == "s3":
        ...
        from app.modules.backups.backup_destination import destination_for_ownership

        if meta.source_ref != backup_targets.source_reference(
...
def _reverify_backup(run: GcRun) -> None:
    ...
    verification = backup_verification.verify_backup(
        run.backup_id, source_ref=run.backup_source_ref
    )
    if not verification.valid or not verification.app_compatible:
        raise GcSafetyError("gc_backup_witness_invalid")
    _require_unchanged_identity(run, source)
