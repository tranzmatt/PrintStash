...
from app.modules.sources import library_source
from app.modules.sources.library_source import (
    LibrarySourceError,
    RemoteLibrarySource,
    SourceEntry,
)
from app.modules.storage.remote_io import RemoteEntry as SourceDirectoryEntry
...
class _StableDirectoryBackend:
    ...
    def open_reader(self, key, *, expected=None):
        from app.modules.storage.remote_io_adapters import _ChunkReader
...
class TestSourceProbe:
    def test_source_probe_closes_after_one_directory_observation(self):
        calls = []
                ...
                def entries():
                    try:
                        for index in range(100_000):
                            calls.append(index)
                            yield SourceDirectoryEntry(f"{index}.gcode", 3, False)
                    finally:
                        calls.append("closed")
        ...
        assert RemoteLibrarySource(Backend()).probe() == 1
