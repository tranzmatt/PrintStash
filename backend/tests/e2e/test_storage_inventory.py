...
@pytest.mark.asyncio
async def test_explicit_cleanup_refreshes_storage_insights(
    api, e2e_db, superuser_headers, tmp_path
):
    ...
    assert (
        e2e_db.exec(
            select(AuditLog).where(AuditLog.action == "storage.cleanup_expired_staging")
        ).first()
        is not None
    )
