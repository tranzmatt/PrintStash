...
def test_serializes_postgres_capacity_admission():
    url = postgres_url()
    run_migrations(url)
    ...
    def reserve(owner):
        ...
        try:
            manager.reserve(
                owner, [CapacityResource.for_quota(domain, 60, 100, role="test")]
            )
            return True
...
def test_postgres_capacity_upgrade_preserves_models():
