"""Defend audit-policy migrations across populated SQLite and PostgreSQL upgrades."""
        ...
        try:
            ...
            with engine.begin() as connection:
                ...
                assert (
                    connection.execute(
                        text(
                            "SELECT event_type FROM notification_deliveries WHERE id=1"
                        )
                    ).scalar_one()
                    == "PRINT_COMPLETED"
                )
                ...
                assert (
                    connection.execute(
                        text(
                            "SELECT event_type FROM notification_deliveries WHERE id=2"
                        )
                    ).scalar_one()
                    == "STORAGE_REGRESSION"
                )
                connection.execute(
                    text("DELETE FROM notification_deliveries WHERE id=2")
                )
