...
def test_pause_preserves_due_slot(db_session, make_user, make_audit_policy):
    ...
    update_policy(db_session, VaultAuditMode.QUICK, {"paused": True}, user.id, now=NOW)
    assert ensure_utc(policy.next_due_at) == NOW
...
def test_manual_success_satisfies_due_slot(
    ...
    user = make_user()
    policy = make_audit_policy(user, enabled=True, next_due_at=NOW)
    run = make_audit_run(user, started_at=NOW, finished_at=NOW + timedelta(minutes=1))
...
def test_estimates_known_remote_bytes(db_session, make_owned_storage_object):
    ...
    make_owned_storage_object(
        backend="s3", size_bytes=100, object_kind="artifact", key="files/source.gcode"
    )
    make_owned_storage_object(
        backend="s3", size_bytes=20, object_kind="thumbnail", key="thumbs/source.webp"
    )
    make_owned_storage_object(backend="local", size_bytes=500, object_kind="artifact")
...
def test_enforces_stream_bandwidth(
    ...
    monkeypatch.setattr(
        vault_audit, "time", SimpleNamespace(monotonic=lambda: elapsed[0], sleep=sleep)
    )
