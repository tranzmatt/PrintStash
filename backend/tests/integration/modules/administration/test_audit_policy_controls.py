...
def test_stale_editor_cannot_overwrite_policy(db_session, make_user, make_audit_policy):
    user = make_user()
...
def test_jitter_is_stable_and_keeps_a_minute_in_window(make_user, make_audit_policy):
...
def test_overdue_is_durable_once_and_preserves_due(
    db_session,
    make_user,
    make_audit_policy,
    make_system_config,
...
def test_terminal_notifications_commit_with_result(
    ...
    from app.db.models import NotificationDelivery, VaultAuditEvent, VaultAuditRunState
...
def test_retention_preserves_baselines_and_event_evidence(
    ...
    assert {row.run_id for row in db_session.exec(select(VaultAuditFinding)).all()} == {
        runs[1].id,
        runs[2].id,
    }
...
def test_failed_repair_retains_finding_and_safe_event(
    db_session,
    local_storage,
    make_user,
    make_model,
    ...
    run = make_audit_run(
        make_user(), repair_actions_json='["reparse_metadata"]', finished_at=utcnow()
    )
