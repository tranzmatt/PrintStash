...
from app.modules.ingestion.ingestion import resolve_write_target
...
from tests.integration.modules.sources.external_library._helpers import drop_gcode
...
class TestExternalRootBinding:
    def test_changed_source_is_rejected_before_thumbnail_processing(
        ...
        try:
            with pytest.raises(source_root_binding.ExternalRootBindingError) as caught:
    ...
    def test_matching_orphan_marker_requires_explicit_reenrollment(
        ...
        marker = {
            "format": root_markers.ROOT_MARKER_FORMAT,
            "installation": "a" * 64,
            "role": root_markers.ROOT_MARKER_ROLE,
        ...
        assert source_root_binding.root_binding_state(library) == (
            "unbound",
    ...
    def test_legacy_marker_read_failure_stays_fail_closed(
        ...
        monkeypatch.setattr(source_root_binding, "_read_root_marker", fail)

        assert source_root_binding.root_binding_state(library) == expected
    ...
    def test_nonobject_marker_result_never_binds_a_root(
        ...
        monkeypatch.setattr(source_root_binding, "_read_root_marker", lambda _root: [])
    ...
    def test_oversized_marker_never_binds_a_root(
        ...
        marker = root / root_markers.ROOT_MARKER_FILENAME
    ...
    def test_enrollment_rejects_an_unreadable_existing_marker(
        ...
        with pytest.raises(source_root_binding.ExternalRootBindingError) as caught:
            source_root_binding.enroll_external_root(db_session, library)
    ...
    def test_enrollment_never_overwrites_a_concurrent_marker(
        ...
        monkeypatch.setattr(source_root_binding, "_create_marker", lambda *_args: False)
    ...
    def test_marker_payload_rejects_noncanonical_identity(
        ...
        with pytest.raises(ValueError, match="root_marker_invalid"):
            root_markers._validate_marker_payload(payload)  # noqa: SLF001
    ...
    def test_markerless_root_can_be_explicitly_enrolled(
        ...
        state, reason = source_root_binding.root_binding_state(library)
        assert (state, reason) == ("bound", None)
        marker = json.loads(
            (root / root_markers.ROOT_MARKER_FILENAME).read_text(encoding="utf-8")
        )
    ...
    def test_matching_orphan_marker_can_be_explicitly_reenrolled(
        ...
        marker_path = root / root_markers.ROOT_MARKER_FILENAME
        marker_path.write_text(json.dumps(marker), encoding="utf-8")

        source_root_binding.enroll_external_root(db_session, library)

        assert library.root_identity == marker["root_identity"]
        assert source_root_binding.root_binding_state(library) == ("bound", None)
    ...
    def test_markerless_replacement_requires_token_rotation(
        ...
        (root / root_markers.ROOT_MARKER_FILENAME).unlink()
