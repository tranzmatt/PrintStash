...
class TestSpoolmanClient:
    def test_list_spools_unwraps_the_providers_envelope(self):
        ...
        with patch("app.modules.printing.spoolman.get_http_client") as mock_get_client:
...
class TestRecordSpoolUsage:
    ...
    def test_decrements_when_configured(self, db_session: Session):
        ...
        with (
            patch("app.modules.printing.spoolman.active_spool_sync", return_value=None),
            patch("app.modules.printing.spoolman.use_spool_weight_sync") as mock_use,
        ):
    ...
    def test_skips_when_native_hook_active(self, db_session: Session):
        ...
        job = _completed_job(spool_id=3, grams=10.0)
        with (
            patch("app.modules.printing.spoolman.active_spool_sync", return_value=7),
            patch("app.modules.printing.spoolman.use_spool_weight_sync") as mock_use,
        ):
    ...
    def test_force_overrides_native_hook(self, db_session: Session):
        ...
        with (
            patch(
                "app.modules.printing.spoolman.active_spool_sync", return_value=7
            ) as mock_active,
            patch("app.modules.printing.spoolman.use_spool_weight_sync") as mock_use,
        ):
            assert print_results.record_spool_usage(db_session, job) is True
    ...
    def test_noop_without_spool(self, db_session: Session):
        ...
        with patch("app.modules.printing.spoolman.use_spool_weight_sync") as mock_use:
            assert print_results.record_spool_usage(db_session, job) is False
            mock_use.assert_not_called()
    ...
    def test_swallows_spoolman_error(self, db_session: Session):
        ...
        with (
            patch("app.modules.printing.spoolman.active_spool_sync", return_value=None),
            patch(
                "app.modules.printing.spoolman.use_spool_weight_sync",
...
class TestUseSpoolWeightSync:
    def test_raises_on_non_2xx(self):
        with patch("app.modules.printing.spoolman.httpx.put") as mock_put:
...
class TestActiveSpoolSync:
    def test_returns_parsed_value(self):
        with patch("app.modules.printing.spoolman.httpx.get") as mock_get:
