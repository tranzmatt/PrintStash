...
from app.modules.storage.storage_backend import local as storage_local
...
class TestVerifyDestructiveAccess:
    ...
    def test_probes_every_distinct_parent(
        ...
        second = owned()
        probed: list[str] = []
        real_mkstemp = storage_local.tempfile.mkstemp

        def recording_mkstemp(*args: object, **kwargs: object):
            probed.append(str(kwargs.get("dir")))
            return real_mkstemp(*args, **kwargs)

        monkeypatch.setattr(storage_local.tempfile, "mkstemp", recording_mkstemp)
    ...
    def test_refuses_a_root_it_cannot_write_to(
        ...
        monkeypatch.setattr(storage_local.tempfile, "mkstemp", read_only)
