...
from app.modules.ingestion import importer as imp
from app.modules.ingestion.ingestion import _collision_safe_path
from app.modules.printing.moonraker import MoonrakerClient
from app.modules.sources.external_library import _collection_path_for, _walk, is_due
...
class TestCollectionForArchive:
    def test_nests_under_parent(self) -> None:
        assert (
            imp.archive_collection_path("Functional", "Brackets.zip")
            == "Functional/Brackets"
        )

    def test_no_parent_uses_archive_stem(self) -> None:
        assert imp.archive_collection_path(None, "MyPack.zip") == "MyPack"

    def test_blank_parent_is_ignored(self) -> None:
        assert imp.archive_collection_path("   ", "Pack.zip") == "Pack"

    def test_trailing_slash_on_parent_collapses(self) -> None:
        assert imp.archive_collection_path("A/B/", "Pack.zip") == "A/B/Pack"
...
class TestMatchingFilamentProfile:
    def test_exact_name_match_case_insensitive(self) -> None:
        ...
        assert models_costing.match_cost_profile(_profiles(), md).name == "Hatchbox PLA"
    ...
    def test_type_only_matches_brandless_profile(self) -> None:
        ...
        assert models_costing.match_cost_profile(_profiles(), md).name == "Generic PETG"

    def test_no_match_returns_none(self) -> None:
        md = Metadata(file_id=1, material_type="ABS")
        assert models_costing.match_cost_profile(_profiles(), md) is None
...
class TestFilamentCostForGrams:
    def test_cost_scales_per_kg(self) -> None:
        ...
        assert models_costing.filament_cost_for_grams(_profiles(), md, 100.0) == 2.0

    def test_none_grams_or_metadata(self) -> None:
        md = Metadata(file_id=1, material_type="PLA", material_brand="Hatchbox")
        assert models_costing.filament_cost_for_grams(_profiles(), md, None) is None
        assert models_costing.filament_cost_for_grams(_profiles(), None, 100.0) is None

    def test_profile_without_cost_returns_none(self) -> None:
        md = Metadata(file_id=1, material_type="PLA", material_brand="NoCost")
        assert models_costing.filament_cost_for_grams(_profiles(), md, 100.0) is None
...
class TestCsvCell:
    ...
    def test_keeps_real_values_including_zero(self, value) -> None:
        ...
        assert models_exports._csv_cell(value) == value

    def test_none_becomes_empty(self) -> None:
        assert models_exports._csv_cell(None) == ""
...
class TestFilamentProfileUsage:
    def test_counts_live_files_matching_each_profile(self, db_session: Session) -> None:
        ...
        usage = models_costing.filament_profile_usage(db_session)
...
class TestPrinterProfileUsage:
    def test_counts_live_files_matching_printer_model_or_preset_name(
        ...
        usage = models_costing.printer_profile_usage(db_session)
    ...
    def test_blank_printer_model_is_skipped(self, db_session: Session) -> None:
        ...
        assert models_costing.printer_profile_usage(db_session) == {}
